...
Viewport bucket, one expression shared by the aggregator and the live
half (`sqlite.viewportBucketSQL`):

```sql
CASE WHEN viewport_width < 640  THEN '<640'
     WHEN viewport_width < 1024 THEN '640-1023'
     WHEN viewport_width < 1440 THEN '1024-1439'
     WHEN viewport_width < 1920 THEN '1440-1919'
     ELSE '>=1920' END
```
...
viewport expression as §7 — the app views never did, which meant an app
...
`009_views.sql`, applied by the existing runner in one transaction.
...
3. `actors`: rebuild with `actor_kind = CASE surface WHEN 'app' THEN
   'install' ELSE 'user' END`; same mapping for `agg_retention`. Old web
   cohorts on identified projects therefore keep whatever connection-hash
...
enforce it. A dedicated `TestMigration009Folds` seeds a database at
schema 008 with known rows in every old table, runs the migration and
...
- Store: `aggregate_views_test.go` (sessions with and without client
  ...
  cap); stitch invariants per §8; `TestMigration009Folds`; prune and
