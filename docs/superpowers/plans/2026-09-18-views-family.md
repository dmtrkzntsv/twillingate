...
**Architecture:** One raw table `views`, one aggregator `AggregateViewDay`, eleven `v_views_*` stitch views, two MCP/REST tools, one Evidence page. Kind is a value on the row (`web`, `app`, `cli`, …); only `web` is User-Agent parsed and bot-filtered. Retention keys on how the actor was identified (`actor_kind`), recorded at ingest. Migration 009 folds both old families and their aggregates into the new tables in one transaction.
...
- Never edit migrations 001–008. All schema change is `009_views.sql`.
...
### Task 2: Store types, migration 009 and the write path
...
- Create: `internal/store/sqlite/migrations/009_views.sql`
...
- [ ] **Step 2: Write `009_views.sql`**

Create `internal/store/sqlite/migrations/009_views.sql`. The runner wraps the whole file in one transaction (`migrate.go`), so a failure midway leaves the old schema intact.
...
git commit -m "feat(store): one views table and migration 009 folding the web and app families"
```
...
- Modify: `internal/store/sqlite/migrations/009_views.sql` (append the views), `internal/store/sqlite/views_test.go`
...
// actor splits sessions. The live half of v_views_daily (009_views.sql)
...
Run: `go test ./internal/store/sqlite/ -run 'TestAggregateViewDay|TestViewDaysBefore'`
Expected: FAIL only on missing tables/views if 009's views are not yet appended (Step 5); the daily and dimension assertions themselves should pass. If `views_test.go` or other files still reference deleted symbols, fix those compile errors first by applying Steps 5–6 of this task.

- [ ] **Step 5: Append the stitch views to `009_views.sql`**

Append to the end of `internal/store/sqlite/migrations/009_views.sql`. The live halves must stay numerically identical to `AggregateViewDay`; `views_test.go` enforces it.
...
### Task 5: Migration 009 fold test

Spec §11 ("A dedicated `TestMigration009Folds`"). The test opens a database migrated only through 008, seeds every old table, runs 009 and asserts the folded totals.

**Files:**
- Modify: `internal/store/sqlite/migrate.go`
- Create: `internal/store/sqlite/migration009_test.go`
...
func TestMigration009Folds(t *testing.T) {
	db := newTestDBAt(t, 8)
	ctx := context.Background()
...
Run: `go test ./internal/store/sqlite/ -run 'TestMigration009Folds|TestMigrationsAreIdempotent|TestMigrationViews|TestProjectTablesMatchesSchema|TestPruneAggregatesCoversAllAggTables'`
Expected: PASS. If the `$os` merge asserts (5,5) fails with (3,3) or (2,2), the `INSERT OR REPLACE … FROM os_attrs` ran against a temp table that did not group — check the `GROUP BY … 5` ordinal in 009.

- [ ] **Step 4: Commit**

```bash
git add internal/store
git commit -m "test(store): prove migration 009 folds every old table into the views family"
```
...
Then boot the binary against a fresh database and against a copy of a pre-merge database to see 009 apply:
...
Expected: exits 0; `twillingate migrate` on a copy of an existing `twillingate.db` (for example `local/twillingate.db` after `make seed-demo` on `main`) also exits 0 and the log names migration 009.
