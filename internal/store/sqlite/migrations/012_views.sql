...
--
-- Folds from the post-011 shapes: actors carries is_user and agg_retention
-- carries users (011), and v_identity_daily already skips aggregated days
-- (010) -- both are carried through below.
--
...
-- actor_kind is how the actor was identified, not which surface it arrived
-- on, so 011's is_user decides it for web, app and product alike: an actor
-- that ever carried a user_id is a user, everything else is an install.
INSERT INTO actors_new SELECT project, actor_id,
       CASE WHEN is_user = 1 THEN 'user' ELSE 'install' END,
...
-- 011's rows are keyed by surface and carry users (NULL where the count
...
-- 010 already deleted the partial rows the old pass wrote for today; the
-- pass no longer writes them, so there is nothing to repeat here.
...
-- Unlike the other stitch views, this aggregate is written while the day's
-- raw rows still exist: the daily pass rolls identity up over every raw day
-- (jobs.go), and only AggregateViewDay deletes raw -- product events stay
-- for their own raw window. The live half therefore has to skip days the
-- aggregate already holds (010), rather than rely on the two being disjoint.
--
-- The aggregate is preferred over raw for a day that has both: raw windows
-- differ per table, so once views for a day are pruned but its product
-- events are not, the live half would silently lose the views share. The
