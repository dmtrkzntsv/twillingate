...
func (h *host) integrationGuide(ctx context.Context, in guideIn) (guideOut, error) {
	...
	case "web", "spa":
		...
		b.WriteString(". Nothing is filtered on the client: a localhost page reports too, so keep development traffic out with init({ optOut: () => location.hostname === \"localhost\" }).\n\n")
